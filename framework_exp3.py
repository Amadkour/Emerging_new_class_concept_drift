...
streams = helper.synthetic_streams()
...
def worker(i, stream_n):
    ...
    for threshold in thresholds:
        ...
        for index, algorithm in enumerate(algorithms):
            ...
            print(f"==========(%s)===with threshold (%s)======finished in {finish_time - start_time} seconds" % (
            algorithm, threshold), )
        ...
        np.save(f"output/result1/%s/strean_learning/%s" % (classifiers_name[0], threshold), results)
