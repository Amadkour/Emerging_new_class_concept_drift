...
knn_u1 = SEA(base_estimator=StratifiedBagging(base_estimator=KNeighborsClassifier()), des="None",)
knn_e1 = SEA(base_estimator=StratifiedBagging(base_estimator=KNeighborsClassifier(), ), des="None")
knn_u2 = SEA(base_estimator=StratifiedBagging(base_estimator=KNeighborsClassifier()), des="None")
knn_e2 = SEA(base_estimator=StratifiedBagging(base_estimator=KNeighborsClassifier(), ), des="None")
...
def worker(i, stream_n):
    ...
    for index, classifer_name in enumerate(classifiers_name):
        ...
        for method in drift_methods:
            ...
            results = eval.scores
            np.save("output/%s" % (str(stream).split('/')[1].split('.')[0]),results)
...
if __name__ == '__main__':
    ...
    result = Parallel(n_jobs=1, prefer="threads")(delayed(worker)(i, stream_n) for i, stream_n in enumerate(streams))
